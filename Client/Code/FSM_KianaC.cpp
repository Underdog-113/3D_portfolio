...
void CFSM_KianaC::Attack_1_Update(float deltaTime)
{
	...
	if (CheckAction_Attack(Name_Attack_2, 0.3f))
		return;
	if (CheckAction_WeaponSkill(0.3f))
		return;
...
void CFSM_KianaC::Attack_3_Branch_Update(float deltaTime)
{
	...
	{
		m_pKiana->ActiveAttackBall(1.f, HitInfo::Str_Airborne, HitInfo::CC_None, m_pKiana->GetRightToeWorldMatrix(), 0.3f);
		m_checkAttack = true;
	}
