...
VS_OUTPUT vs_main(VS_INPUT Input)
{
	...
	Output.mUV = Input.mUV + float2(gTime, 0);
...
float4 ps_main(PS_INPUT Input) : COLOR
{
	...
	float4 Noise = tex2D(NoiseTex, Input.mUV + float2(gTime,0));
	if(Input.mUV.y >= gAlpha)
		albedo.a = 0;
	...
	return float4((albedo * Noise) * diffuse.xyz, gAlpha);
}
